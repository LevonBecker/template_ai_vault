...
history. A real repo forked from a template (like ai_vault, this repo's own leaf descendant) gets
the ignore line stripped from .gitignore the first time setup runs there instead, so properties.yml
is committed like any other repo config (see `_sync_gitignore_tracking()` below). It's assembled
from every tier fragment under
...
def _sync_gitignore_tracking(repo_remote: str | None) -> None:
    ...
    from one (like ai_vault, this repo's own leaf descendant) should commit it instead, so this
    removes the ignore line (and its explanatory comment) the first time setup runs there.
    Idempotent: a repo that's already had the line removed, or never had it, is left untouched.
    """
