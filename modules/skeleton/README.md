...
tooling updates from that repo into other projects (including this one) that were bootstrapped
from it.

## Usage
```sh
uv run --no-sync python -m modules.skeleton.route
```
...
`/sync-setup` here. If the local path isn't found (e.g. a different machine or CI), it shallow-
