...
Usage:
    uv run --no-sync python -m modules.skeleton.sync
"""
...
from ..common.properties import get_repo_local, get_skeleton_local, get_skeleton_remote
...
def _clone_remote(remote: str) -> Path:
    ...
    dest = get_repo_local() / "tmp" / "skeleton_sync"
    if dest.exists():
        shutil.rmtree(dest)
