...
@task
def cleanup(context, confirm=True):
    """Delete merged local branches (--confirm/--no-confirm, default: confirm)"""
    flag = "--confirm" if confirm else "--no-confirm"
    context.run(f"python -m modules.repo.cleanup {flag}")


@task
def pull(context):
    """Pull updates from git remote (stash → pull --rebase → restore)"""
    context.run("python -m modules.repo.pull")
...
def push(context, no_confirm=False):
    ...
    context.run(f"python -m modules.repo.push{flag}")
...
def pr_notes_save(context, content=None):
    ...
    context.run(f"python -m modules.repo.pr_notes{flag}")
...
def pr_create(context, title=None, content=None):
    ...
    context.run(f"python -m modules.repo.pr_create{flags}")


@task
def pr_cleanup(context):
    """Switch to the default branch, pull, and delete the merged local feature branch"""
    context.run("python -m modules.repo.pr_cleanup")


@task
def set_screenshots(context):
    """Set up the screenshots/ workflow for this repo"""
    context.run("python -m modules.repo.set_screenshots")


@task
def view_screenshot(context):
    """View the most recent screenshot"""
    context.run("python -m modules.repo.view_screenshot")
