...
Major.Minor.Patch but it hasn't shipped yet. See `versioning.instructions.md` for the full rules.
...
@click.command()
@click.option("--release", is_flag=True, help="Finalize VERSION for release (drop the build suffix)")
def main(release: bool = False) -> None:
    """Bump VERSION for a dev build, or finalize it for release with --release."""
    if release:
        bump_release()
    else:
        bump_build()
