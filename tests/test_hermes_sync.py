...
class TestPromptCommandParser:
    ...
    def test_exec_no_args_classified_exec(self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _write_prompt(prompts_dir, "pull", "Pull from git",
    ...
    def test_arg_style_classified_arg(self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _write_prompt(prompts_dir, "topic", "Manage topics",
    ...
    def test_ai_guided_no_exec_line(self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _write_prompt(prompts_dir, "add_expense", "Add expense",
                      "Collect vendor, amount, description from user.")
        sync = _load_sync_module(prompts_dir, monkeypatch)
        cmds = sync.load_commands()
        assert cmds[0].classification == "ai_guided"

    def test_long_running_test_classified_exec_long(
    ...
    def test_argument_hint_parsed(self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _write_prompt(prompts_dir, "chat", "Manage chats",
    ...
    def test_deduplication_keeps_first_alphabetically(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """list-expenses and list_expenses deduplicate — only one entry kept."""
        _write_prompt(prompts_dir, "list_expenses", "List expenses",
...
class TestQuickCommandsGenerator:
    ...
    def test_only_exec_commands_included(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "pull", "Pull", '!`uv run --no-sync python -m modules.repo.route "pull"`')
    ...
    def test_exec_long_not_in_quick_commands(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "test", "Run tests", "!`uv run --no-sync invoke test`")
        sync = _load_sync_module(prompts_dir, monkeypatch)
    ...
    def test_quick_command_entry_has_type_and_command(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "ss", "Screenshot",
    ...
    def test_exec_override_applied_for_fix(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_command_prefixed_with_cd_repo_root(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "pull", "Pull",
                      '!`uv run --no-sync python -m modules.repo.route "pull"`')
        sync = _load_sync_module(prompts_dir, monkeypatch)
        cmds = sync.load_commands()
        qc = sync.generate_quick_commands(cmds)
...
class TestSkillMdGenerator:
    ...
    def test_exec_commands_appear_in_table(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "pull", "Pull from git",
                      '!`uv run --no-sync python -m modules.repo.route "pull"`')
    ...
    def test_arg_commands_have_subsection(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "topic", "Manage topics",
                      '!`uv run --no-sync python -m modules.topic.route "$ARGUMENTS"`')
    ...
    def test_ai_guided_commands_in_workflow_section(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "add_expense", "Add expense",
                      "Collect vendor from user first.")
        sync = _load_sync_module(prompts_dir, monkeypatch)
        cmds = sync.load_commands()
        md = sync.generate_skill_md(cmds)
    ...
    def test_long_running_commands_in_long_section(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "test", "Run tests", "!`uv run --no-sync invoke test`")
        sync = _load_sync_module(prompts_dir, monkeypatch)
    ...
    def test_ss_vision_note_always_present(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "pull", "Pull", '!`uv run --no-sync python -m modules.repo.route "pull"`')
        sync = _load_sync_module(prompts_dir, monkeypatch)
    ...
    def test_footer_contains_regen_instruction(
        self, prompts_dir: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _write_prompt(prompts_dir, "pull", "Pull", '!`uv run --no-sync python -m modules.repo.route "pull"`')
        sync = _load_sync_module(prompts_dir, monkeypatch)
