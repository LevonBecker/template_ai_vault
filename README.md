...
The Big Win: a single, portable home for our AI-assisted work — one we fully own, that works with whichever AI provider, model, or local tool we reach for today and whatever new one comes along tomorrow.
...
agent-facing rules and the slash-command / invoke-task reference live in `.fireball_ai_toolkit/`.
...
architecture live in **`.fireball_ai_toolkit/`** (rendered per tool into `.claude/`, `.clinerules/`,
...
package — vendored into `modules/fireball_ai_toolkit/` + `tasks/fireball_ai_toolkit/` and refreshed with
