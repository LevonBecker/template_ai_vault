...
def end(context):
    ...
    context.run("python -m modules.chat.end")
...
def list_chats(context, sort="newest_first"):
    ...
    context.run(f"python -m modules.chat.list --sort={sort}")
...
def resume(context, pattern=None):
    ...
    context.run(f"python -m modules.chat.resume{flag}")
...
def start(context, title=None):
    ...
    context.run(f"python -m modules.chat.start{flag}")
